...
public class TERenderer {
    ...
    public void renderFrame(TETile[][] world, int level, String name) {
        int numXTiles = world.length;
        int numYTiles = world[0].length;
        StdDraw.clear(new Color(0, 0, 0));
        for (int x = 0; x < numXTiles; x += 1) {
        ...
        headUpDisplay(world, level, name);
        StdDraw.show();
    }

    public void headUpDisplay(TETile[][] world, int level, String name) {

        StdDraw.setPenColor(Color.WHITE);
        StdDraw.setFont(new Font("Monoco", Font.BOLD, 15));
        StdDraw.textLeft(1.5, 1.5, "level  " + level);
        StdDraw.setFont(new Font("Monoco", Font.PLAIN, 15));
        StdDraw.textRight(Engine.WIDTH - 3, 38.5, "Name: " + name);
    }
}
